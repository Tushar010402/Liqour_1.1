...
func main() {
	...
	// Initialize services
	vendorService := services.NewVendorService(db, redisCache)
	expenseService := services.NewExpenseService(db, redisCache)
	assistantManagerService := services.NewAssistantManagerService(db, redisCache)
	cashService := services.NewCashService(db)
	...
	cashHandlers := handlers.NewCashHandlers(cashService, bankAccountService)
	cashHandlers.SetNotificationService(notificationService) // Wire up FCM notifications
	bankHandlers := handlers.NewBankAccountHandlers(bankAccountService)
	...
	// Setup routes (using SetupProtectedRoutes since we're behind API Gateway)
	routes.SetupProtectedRoutes(router, cfg, redisCache, financeHandlers, cashHandlers, bankHandlers)
