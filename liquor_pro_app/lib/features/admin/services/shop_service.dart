...
class ShopService {
  ...
  /// Get all shops for the current user (role-based)
  /// - Admin/Manager/Executive: All tenant shops
  /// - Salesman: Only assigned shop
  Future<ApiResponse<List<Shop>>> getShops() async {
    final response = await _apiService.get<List<Shop>>(
      '/api/shops', // New role-based endpoint
      fromJson: (data) {
        if (data is List) {
  ...
  Future<ApiResponse<void>> deleteShop(String shopId) async {
