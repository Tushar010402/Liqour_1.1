...
/// Submit Cash Screen - Settings-style UI consistency pattern
/// User submits cash to bank with denomination breakdown
class SubmitCashScreen extends ConsumerStatefulWidget {
  final String shopId;

  /// Optional: Pre-fill form with rejected submission for re-submission
  final CashSubmission? resubmitFrom;

  const SubmitCashScreen({
    super.key,
  ...
  @override
  ConsumerState<SubmitCashScreen> createState() => _SubmitCashScreenState();
}

class _SubmitCashScreenState extends ConsumerState<SubmitCashScreen>
  ...
  void initState() {
    ...
    // Pre-fill from rejected submission if provided (re-submission flow)
    _prefillFromRejectedSubmission();
  }

  /// Pre-fill form with rejected submission data for easy re-submission
  void _prefillFromRejectedSubmission() {
    final resubmit = widget.resubmitFrom;
    if (resubmit == null) return;

    // Pre-fill denomination controllers
    ...
    // Pre-fill bank slip number if available
    ...
    // Pre-fill deposit date
    if (resubmit.depositDate != null) {
      _depositDate = resubmit.depositDate!;
    }

    // Note: Bank account selection will be handled in didChangeDependencies
    // after bank accounts are loaded
  }
  ...
  void _autoFill(double availableBalance) {
    // Smart algorithm to fill denominations optimally
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: const CustomAppBar(title: 'Submit to Bank'),
      body: cashBalance.when(
        data: (balance) {
          if (balance <= 0) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // Counter Controls - Compact & Responsive
          Expanded(
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget _buildBankAccountSelector() {
    ...
    return bankAccountsAsync.when(
      data: (accounts) {
        // Auto-select default account if available and nothing is selected
                        ...
                        child: Column(
                          ...
                          children: [
                            if (_selectedBankAccount != null) ...[
                              ...
                              Text(
                                'Available: ${Formatters.currency(_selectedBankAccount!.totalAvailableBalance)}',
                                style: const TextStyle(
                                  fontSize: 14,
                                  fontWeight: FontWeight.w600,
                                  color: Colors.green,
            ...
            Expanded(
              child: CupertinoPicker(
                ...
                onSelectedItemChanged: (int index) {
                  setState(() {
                    _selectedBankAccount = accounts[index];
                  });
                },
                  ...
                  return Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Available: ${Formatters.currency(account.totalAvailableBalance)}',
                          style: const TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w600,
                            color: Colors.green,
  ...
  Widget _buildSubmitButton(double availableBalance) {
    final cs = Theme.of(context).colorScheme;
    final isValid = _calculatedTotal > 0 &&
        _calculatedTotal <= availableBalance &&
        _receiptImage != null;
  ...
  Future<void> _scanBankSlip() async {
    ...
    if (result.success && result.processedImages.isNotEmpty) {
      // Show preview screen for user to review and accept/retake
      if (!mounted) return;
      ...
      final accepted = await Navigator.push<bool>(
        ...
        MaterialPageRoute(
          builder: (_) => DocumentPreviewScreen(
            imageFile: result.processedImages.first,
            title: 'Review Bank Slip',
          ),
        ),
      );
      ...
      if (accepted == true && mounted) {
        ...
        // User wants to retake - recursive call
        _scanBankSlip();
      }
      // If null (back button pressed), do nothing
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      final receiptUrl =
          await cashNotifier.uploadReceiptPhoto(_receiptImage!.path);
      ...
      final request = SubmitCashRequest(
        ...
        bankAccountId: _selectedBankAccount?.id, // Include bank account ID
        bankSlipNumber: _bankSlipController.text.trim().isEmpty
            ? null
            : _bankSlipController.text.trim(),
      ...
      if (success && mounted) {
        SnackbarHelper.showSuccess(
          context,
          'Cash submitted successfully! Awaiting approval.',
        );
