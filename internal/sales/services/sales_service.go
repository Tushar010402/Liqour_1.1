...
// SaleItemResponse represents sale item in responses. DisplayName + bold
// indices flow through so the web admin renders the admin-configured bold
// distinctive identifier + smaller light rest on sale detail pages, daily
// sale lists, and the create-sale entry grid.
	...
	if len(sale.Items) > 0 {
		...
		for i, item := range sale.Items {
			...
			if item.Product != nil {
				...
				// Flow display_name through so the web sale-detail renders
				// the admin-configured bold + small treatment consistently
				// with the rest of the admin (Brands, Products, Stock).
