...
import (
	...
	"github.com/liquorpro/go-backend/internal/auth/routes"
...
func main() {
	...
	// Start server
	srv := &http.Server{
		Addr:         fmt.Sprintf("%s:%d", cfg.Server.Host, cfg.Services.Auth.Port),
		Handler:      router,
	...
	go func() {
		log.Printf("Authentication service starting on %s:%d", cfg.Server.Host, cfg.Services.Auth.Port)
	...
	log.Println("Authentication service stopped")
}