...
func (s *ExpenseService) GetCashFlowReport(ctx context.Context, tenantID uuid.UUID, startDate, endDate time.Time, groupBy string) (*CashFlowReport, error) {
	...
	cacheKey := fmt.Sprintf("cash_flow:tenant:%s:start:%s:end:%s:group:%s",
		tenantKeyPart, startDate.Format("2006-01-02"), endDate.Format("2006-01-02"), groupBy)

	var cached CashFlowReport
	if err := s.cache.Get(ctx, cacheKey, &cached); err == nil {
		return &cached, nil
	}

	// Determine date format for grouping
	dateFormat := "YYYY-MM" // monthly default
	switch groupBy {
	case "daily":
	...
	// Query inflow: approved sales revenue grouped by period
	var inflowAggs []periodAgg
	inflowQuery := s.db.DB.Table("daily_sales_records").
		Select(fmt.Sprintf("TO_CHAR(record_date, '%s') as period_label, COALESCE(SUM(total_sales_amount), 0) as total", dateFormat)).
		Where("record_date >= ? AND record_date <= ?", startDate, endDate).
		Where("status = ?", "approved").
		Where("deleted_at IS NULL")
	if tenantID != uuid.Nil {
		inflowQuery = inflowQuery.Where("tenant_id = ?", tenantID)
	}
	inflowQuery.Group("period_label").Order("period_label").Scan(&inflowAggs)

	// Query outflow from expenses grouped by period
	var expenseAggs []periodAgg
	expenseQuery := s.db.DB.Table("expenses").
		Select(fmt.Sprintf("TO_CHAR(expense_date, '%s') as period_label, COALESCE(SUM(amount), 0) as total", dateFormat)).
		Where("expense_date >= ? AND expense_date <= ?", startDate, endDate).
		Where("deleted_at IS NULL")
	if tenantID != uuid.Nil {
		expenseQuery = expenseQuery.Where("tenant_id = ?", tenantID)
	}
	expenseQuery.Group("period_label").Order("period_label").Scan(&expenseAggs)

	// Query outflow from stock purchases grouped by period
	var purchaseAggs []periodAgg
	purchaseQuery := s.db.DB.Table("stock_purchases").
		Select(fmt.Sprintf("TO_CHAR(purchase_date, '%s') as period_label, COALESCE(SUM(total_amount), 0) as total", dateFormat)).
	...
	purchaseQuery.Group("period_label").Order("period_label").Scan(&purchaseAggs)

	// Build maps for quick lookup
	inflowMap := make(map[string]float64)
	for _, a := range inflowAggs {
		inflowMap[a.PeriodLabel] = a.Total
	}
	expenseMap := make(map[string]float64)
	for _, a := range expenseAggs {
		expenseMap[a.PeriodLabel] = a.Total
	}
	purchaseMap := make(map[string]float64)
	for _, a := range purchaseAggs {
		purchaseMap[a.PeriodLabel] = a.Total
	}

	// Collect all unique period labels
	periodSet := make(map[string]bool)
	for k := range inflowMap {
		periodSet[k] = true
	}
	for k := range expenseMap {
		periodSet[k] = true
	}
	for k := range purchaseMap {
		periodSet[k] = true
	}
	...
	for k := range periodSet {
		periodLabels = append(periodLabels, k)
	}
	sortStrings(periodLabels)

	// Assemble periods
	var periods []CashFlowPeriod
	totalInflow, totalOutflow := 0.0, 0.0
	for _, label := range periodLabels {
		inflow := inflowMap[label]
		outflow := expenseMap[label] + purchaseMap[label]
		net := inflow - outflow
	...
	s.cache.Set(ctx, cacheKey, report, 600) // Cache for 10 minutes
...
func (s *ExpenseService) GetProfitLossReport(ctx context.Context, tenantID uuid.UUID, startDate, endDate time.Time) (*ProfitLossReport, error) {
	...
	cacheKey := fmt.Sprintf("profit_loss:tenant:%s:start:%s:end:%s",
		tenantKeyPart, startDate.Format("2006-01-02"), endDate.Format("2006-01-02"))

	var cached ProfitLossReport
	if err := s.cache.Get(ctx, cacheKey, &cached); err == nil {
		return &cached, nil
	}
	...
	// Revenue: SUM of approved daily sales
	var salesRevenue float64
	salesQuery := s.db.DB.Table("daily_sales_records").
	...
	salesQuery.Scan(&salesRevenue)

	report.Revenue.Sales = salesRevenue
	report.Revenue.OtherRevenue = 0
	report.Revenue.Total = salesRevenue

	// COGS: SUM of stock purchases (non-cancelled)
	var cogs float64
	cogsQuery := s.db.DB.Table("stock_purchases").
		Select("COALESCE(SUM(total_amount), 0)").
		Where("purchase_date >= ? AND purchase_date <= ?", startDate, endDate).
		Where("deleted_at IS NULL").
		Where("status != ?", "cancelled")
	if tenantID != uuid.Nil {
		cogsQuery = cogsQuery.Where("tenant_id = ?", tenantID)
	}
	cogsQuery.Scan(&cogs)

	report.Expenses.CostOfGoodsSold = cogs

	// Expenses by category: classify into operating/marketing/admin/other
	type categoryExpense struct {
		CategoryName string  `gorm:"column:category_name"`
		Amount       float64 `gorm:"column:amount"`
	}
	var categoryExpenses []categoryExpense
	catQuery := s.db.DB.Table("expenses e").
		Select("COALESCE(ec.name, 'Uncategorized') as category_name, SUM(e.amount) as amount").
	...
	if tenantID != uuid.Nil {
		catQuery = catQuery.Where("e.tenant_id = ?", tenantID)
	}
	catQuery.Group("ec.name").Scan(&categoryExpenses)

	var operatingExp, marketingExp, adminExp, otherExp float64
	for _, ce := range categoryExpenses {
		nameLower := strings.ToLower(ce.CategoryName)
		switch {
		case containsAny(nameLower, "marketing", "advertising", "promotion"):
			marketingExp += ce.Amount
		case containsAny(nameLower, "admin", "office", "salary", "rent", "utility", "insurance"):
			adminExp += ce.Amount
		case containsAny(nameLower, "transport", "maintenance", "repair", "shop", "operating", "logistics"):
			operatingExp += ce.Amount
		default:
			otherExp += ce.Amount
		}
	}

	report.Expenses.OperatingExpenses = operatingExp
	report.Expenses.MarketingExpenses = marketingExp
	report.Expenses.AdministrativeExp = adminExp
	report.Expenses.OtherExpenses = otherExp
	report.Expenses.Total = cogs + operatingExp + marketingExp + adminExp + otherExp

	// Calculations
	report.GrossProfit = report.Revenue.Total - report.Expenses.CostOfGoodsSold
	report.OperatingProfit = report.GrossProfit - (operatingExp + marketingExp + adminExp + otherExp)
