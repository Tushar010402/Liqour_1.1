...
// Ping checks cache connectivity (alias for Health)
...
const (
	...
	SessionTTL = 365 * 24 * time.Hour // 1 year — Anthropic-style sticky login. Re-set on every /auth/refresh (sliding), so active users effectively never re-login; remains server-revocable by deleting the session key. Used by web (password) AND mobile (OTP/Firebase) login paths.
