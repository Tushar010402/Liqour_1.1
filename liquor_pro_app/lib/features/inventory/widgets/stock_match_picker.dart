...
/// Bottom sheet for disambiguating a low-confidence or ambiguous AI stock item.
...
class StockMatchPicker extends StatefulWidget {
  final SmartStockItem item;

  const StockMatchPicker({
    super.key,
    required this.item,
  });
  ...
  static Future<int?> show(
    ...
    required SmartStockItem item,
  }) {
    return showModalBottomSheet<int>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => StockMatchPicker(item: item),
    );
  }

  @override
  State<StockMatchPicker> createState() => _StockMatchPickerState();
}

class _StockMatchPickerState extends State<StockMatchPicker> {
  ...
  Widget _buildAIInfoCard(SmartStockItem item) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'AI detected: ${item.brandName} | ${item.size} | Qty: ${item.quantity}',
  ...
  Widget _buildCandidateCard(int index, StockAlternativeMatch alt) {
                  ...
                  Row(
                    children: [
                      ...
                      if (alt.currentStock != null) ...[
                        ...
                        Text(
                          'Current stock: ${alt.currentStock}',
                          style: _body(size: 11, color: alt.currentStock! > 0
                              ? const Color(0xFFFFA726) : const Color(0xFF4CAF50)),
                        ),
                      ],
                    ],
                  ),
